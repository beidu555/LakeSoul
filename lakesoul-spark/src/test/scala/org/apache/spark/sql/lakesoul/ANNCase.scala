...
class ANNCase extends QueryTest
  ...
  // val dataset = "glove-200-angular"
  // val dataset = "sift-128-euclidean"
  var numHashes = 256
  var signatureLength = 8
  var bucketLimit = 100000
  var bucketWidth = 16
  var filePath = "/home/zhibin/data/glove-50-angular.hdf5"
  var distanceType = "l2"
  ...
  if (dataset == "fashion-mnist") {
    filePath = "/home/zhibin/data/fashion-mnist-784-euclidean.hdf5"
    numHashes = 256
    signatureLength = 8
    bucketLimit = 2000
    bucketWidth = 20
    ...
    filePath = "/home/zhibin/data/glove-50-angular.hdf5"
    numHashes = 256
    signatureLength = 8
    bucketLimit = 100000
    bucketWidth = 16
    ...
    filePath = "/home/zhibin/data/nytimes-256-angular.hdf5"
    numHashes = 128
    signatureLength = 4
    bucketLimit = 20000
    bucketWidth = 16
    ...
    filePath = "/home/zhibin/data/coco-i2i-512-angular.hdf5"
    numHashes = 128
    signatureLength = 4
    bucketLimit = 10000
    bucketWidth = 16
    normType = "NORM_NONE"
    distanceType = "cosine"
  } else if (dataset == "glove-200-angular") {
    filePath = "/home/zhibin/data/glove-200-angular.hdf5"
    numHashes = 256
    signatureLength = 8
    bucketLimit = 100000
    bucketWidth = 16
    normType = "NORM_NONE"
    distanceType = "cosine"
  } else if (dataset == "sift-128-euclidean") {
    filePath = "/home/zhibin/data/sift-128-euclidean.hdf5"
    numHashes = 256
    signatureLength = 8
    bucketLimit = 100000
    bucketWidth = 16
    ...
    withTempDir { trainDir =>
      withTempDir { testDir =>
        ...
        try {
          ...
          var float2DDataNeighbor = if (dataset == "coco-i2i-512-angular") {
            ...
            case float2DData: Array[Array[Float]] =>
              ...
              spark.time({
                ...
                df.write.format("lakesoul")
                  ...
                  .option(LakeSoulOptions.SHORT_TABLE_NAME, "trainData4")
              ...
              testData match {
                case float2DTestData: Array[Array[Float]] =>
                  ...
                  dfTest.write.format("lakesoul")
                    .option("hashPartitions", "IndexId")
                    .option("hashBucketNum", 4)
                    .option(LakeSoulOptions.SHORT_TABLE_NAME, "testData4")
                ...
                // 注册 UDF 检查是否有相同位置的匹配
                  val threshold = 18
                  spark.udf.register("has_positional_match", 
                    (arr1: Seq[Long], arr2: Seq[Long]) => {
                      arr1.indices.exists(
                        i => {
                          val x = arr1(i)
                          val y = arr2(i)
                          // 计算汉明距离
                          java.lang.Long.bitCount(x ^ y) <= threshold
                        }
                      )
                    }
                  )
                  val candidateCount = spark.time(
                    {spark.sql(
                      """
                      SELECT COUNT(*) AS candidateCount
                      FROM (
                        SELECT
                          test.IndexId AS indexIdTest,
                          train.IndexId AS indexIdTrain,
                          calculateCosineDistance(test.Embedding, train.Embedding) AS distance
                        FROM testData4 test
                        JOIN trainData4 train
                        ON has_positional_match(test.Embedding_LSH, train.Embedding_LSH)
                      ) AS CandidatePairs
                      """
                    ).collect()(0).getAs[Long]("candidateCount")
                    }
                  )

                  println(s"===================第一步过滤后的候选对数量===============: ${candidateCount/numQuery}/${trainSize}") // 打印结果
                  ...
                  val result = spark.time({
                    ...
                    spark.sql(
                      ...
                      WITH CandidatePairs AS (
                        ...
                        FROM testData4 test
                        JOIN trainData4 train
                        WHERE has_positional_match(test.Embedding_LSH, train.Embedding_LSH)
                      )
                      
                      SELECT
                        CAST(indexI dTest AS LONG) AS indexIdTest,
                        CAST(indexIdTrain AS LONG) AS indexIdTrain,
                        distance
                      FROM (
                        SELECT
                          indexIdTest,
          ...
          //             SELECT
          //               CAST(indexIdTest AS LONG) AS indexIdTest,
          //               CAST(indexIdTrain AS LONG) AS indexIdTrain,
          //               distance
          //             FROM (
          //               SELECT
  ...
  //           StructField("Embedding", ArrayType(FloatType), true, new MetadataBuilder()
  //             .putString(LakeSoulOptions.SchemaFieldMetadata.LSH_EMBEDDING_DIMENSION, trainData.asInstanceOf[Array[Array[Float]]](0).length.toString)
  //             .putString(LakeSoulOptions.SchemaFieldMetadata.LSH_BIT_WIDTH, "64")
  //             .putString(LakeSoulOptions.SchemaFieldMetadata.LSH_RNG_SEED, "1234567890").build()),
  //           StructField("Embedding_LSH", ArrayType(LongType), true)
  //         ))
  //         trainData match {
  ...
  //                   FROM (
  //                       SELECT
  //                           testData4.IndexId AS indexIdTest,
  //                           trainData4.IndexId AS indexIdTrain,
  //                           testData4.Embedding as EmbeddingTest,
  //                           trainData4.Embedding as EmbeddingTrain,
  //                           ROW_NUMBER() OVER(PARTITION BY testData4.IndexId ORDER BY calculateHammingDistance(testData4.Embedding_LSH, trainData4.Embedding_LSH) asc) AS rank
  //                       FROM testData4
  //                       JOIN trainData4
  //                   ) ranked
  ...
  //   println(s"test time is :${endTime1 - startTime1}")
  //   // 使用新的通用函数计算召回率
  ...
  //   withTempDir { dir =>
  //     val tableName = "test_lsh_index_1"
  ...
  //   withTempDir { dir =>
  //     val tableName = "test_lsh1_index_temp"
  ...
  //     println("Schema of explodedDF before saving:")
  //     explodedDF.printSchema()
  //     println(s"total count is ${explodedDF.count()}")
  //     println("================ Saving LSH Index ==================")
  //     val startTime1 = System.currentTimeMillis()
  //     spark.time({
  //       // Save as LakeSoul table
  ...
  //     })
  //     val endTime1 = System.currentTimeMillis()
  //     println(s"time for saving ${endTime1 - startTime1}")
  ...
  //   withTempDir { dir =>
  //     val tableName = "test_lsh_index_2"
  ...
  //     })
  //     val endTime1 = System.currentTimeMillis()
  //     println(s"save time is ${endTime1 - startTime1}")
  ...
  //     println(s"search time is ${endTime2 - startTime2}")
